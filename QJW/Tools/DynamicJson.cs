...
namespace QJW.Tools
{
    public class DynamicJson : DynamicObject
    {
        ...
        private static object CreateJsonNode2(object obj)
        {
            var type = GetJsonType(obj);
            switch (type)
            {
                ...
                case JsonType.boolean:
                    return obj.ToString().ToLower();
                case JsonType.@object:
                    return CreateXObject2(obj);
                case JsonType.array:
                    return CreateXArray2(obj as IEnumerable);
                case JsonType.@null:
                default:
                    return null;
            }
        ...
        private static IEnumerable<XElement> CreateXArray2<T>(T obj) where T : IEnumerable
        {
            return obj.Cast<object>()
                .Select(o => new XElement("item", CreateTypeAttr(GetJsonType(o)), CreateJsonNode(o)));
        }
        ...
        private static IEnumerable<XElement> CreateXObject2(object obj)
        {
            return obj.GetType()
                .GetProperties(BindingFlags.Public | BindingFlags.Instance)
                .Select(pi => new { Name = pi.Name, Value = pi.GetValue(obj, null) })
                .Select(a => new XElement(a.Name, CreateTypeAttr(GetJsonType(a.Value)), CreateJsonNode2(a.Value)));
        }
